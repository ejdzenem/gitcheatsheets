...
- [`git add [file]`](/git-add.md): Snapshots the file in preparation for versioning, adding it to the staging area.
- [`git status`](/git-status.md): Always a good idea, this command shows you what branch you're on, what files are in the working or staging directory, and any other important information.
- [`git push`](/git-push): Uploads all local branch commits to the remote.
